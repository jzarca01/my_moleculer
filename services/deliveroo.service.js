    ...
    async createAccount(mail, password, voucher) {
      return newTempMail
        ...
        .then(() => {
          ...
          return `Account created ${this.settings.email.toLowerCase()}:${
            this.settings.password
          }`;
        })
      ...
      deliveroo
        ...
        .then(orderDetails => {
          if (
            ...
            this.settings.current_step.current_step
          ) {
            this.setCurrentStep(orderDetails);
            this.broker.call('googlehome.send', {
              message: `Votre livraison en est à l'étape: ${
                orderDetails.title
              }, elle arrivera dans ${orderDetails.eta_minutes} minutes`
            });
          }
